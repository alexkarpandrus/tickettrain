...
(defn llm-doc
  ...
  (str/join
   ...
   ["# tickettrain"
    ...
    "- Credentials are configured for the selected tracker"
    ""
    ...
    "- Preview responses include `approvalContext` with the tracker scope and non-secret mutation settings, plus `repository`, `changeRequest`, `item`, `trackerIntent`, and `changeRequestUpdate`."
    ""
    "## Managed metadata"
    ""
    ...
    "6. Present the exact target, hierarchy, labels, managed changes, and warnings."
    "7. Keep the proposal ID internal. Ask the user to approve the described changes; do not ask them to repeat the ID."
    "8. An affirmative reply immediately after that summary approves only that unchanged proposal. Then run `ttt apply` with the same request and proposal ID."
