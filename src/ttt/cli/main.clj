...
(defn llm-doc
  ...
  (str/join
   ...
   ["# tickettrain"
    ...
    "- Taskwarrior translates native lifecycle values to `open`, `active`, `waiting`, `completed`, or `canceled`; updates preserve lifecycle when `state` is omitted."
    ...
    ""
    "```json"
    "{\"action\":\"create_item\",\"title\":\"Ask Jade for a status update on Project X\",\"description\":\"Follow up this week.\",\"project\":\"Work\",\"labels\":[\"follow-up\",\"waiting\"],\"state\":\"open\",\"priority\":\"high\",\"dueAt\":\"2026-09-30T17:00:00Z\",\"availableAt\":null,\"blockedBy\":[\"APP-100\"]}"
    "```"
    ""
    "```json"
    "{\"action\":\"update_item\",\"item\":\"APP-123\",\"comment\":\"Jade replied. Waiting for the revised timeline.\",\"addLabels\":[\"waiting\"],\"removeLabels\":[\"blocked\"],\"state\":\"waiting\",\"priority\":null,\"blockedBy\":[]}"
    "```"
    ""
    "```json"
    ...
    "- Work-item capabilities vary by tracker: Taskwarrior supports lifecycle, priority, due dates, availability, and blockers; Linear and Jira support lifecycle, priority, due dates, and blockers; Asana supports lifecycle, due dates, availability, and blockers; GitHub Issues supports lifecycle. Unsupported fields or values fail during preview."
    ...
    "2. For `create_change_request`, `update_change_request`, or `close_change_request`, set the exact requested change, then skip tracker searches."
